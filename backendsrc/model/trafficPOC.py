...
class Bus:
    ...
    def __init__(
        self, route, location_index, time_till_next_stop, capacity=50, status=None
    ):
        self.route = route
    ...
    def tick(self):
        if self.status == "Enroute":
            ...
            if self.time_till_next_stop < 0:
                ...
                # Arrived
                self.location_index = (self.location_index + 1) % len(
                ...
                if (
                    self.route.locations[self.location_index].bus_q.qsize()
                    < self.route.locations[self.location_index].bus_max
                ):
                    self.status = "At Stop"
        ...
        for i in range(0, min(self.bus_max, self.bus_q.qsize())):
            if self.bus_q.qsize() != 0:
                ...
                if self.name == bus.route.end_location.name:
                    ...
                    for i in range(
                        0, min(int(1 / Person.board_time), bus.passengers.qsize())
                    ):
                    ...
                    if bus.passengers.qsize() == 0:
                        # Bus to leave
                        bus.status = "Enroute"
                        bus.time_till_next_stop = distance_between(
                            bus.route.locations[bus.location_index],
                            bus.route.locations[
                                (bus.location_index + 1) % len(bus.route.locations)
                            ],
                            bus.route,
                        )
                        buses_left += 1
...
def distance_between(stop1, stop2, route):
    ...
    dist = math.sqrt(
        math.pow((stop1.position[0] - stop2.position[0]), 2)
        + math.pow((stop1.position[1] - stop2.position[1]), 2)
    )
